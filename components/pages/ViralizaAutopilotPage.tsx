...
const platformIcons: { [key: string]: React.FC<any> } = {
    'X': (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>,
    'LinkedIn': (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg>,
    'Blog': (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a4 4 0 0 0-4 4v12a4 4 0 0 0 4 4 4 4 0 0 0 4-4V6a4 4 0 0 0-4-4z"></path><path d="M12 6v12"></path></svg>,
    'TikTok': (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5"></path></svg>,
    'Sistema': (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z" /></svg>,
};
    ...
    if (isClient && !hasAccess('autopilot')) {
        return (
             <div className="relative h-full">
                ...
                <FeatureLockedOverlay featureName="Piloto Automático (Autopromoção)" requiredPlan="Plano Anual" />
            </div>
        )
    }

    const autopilotTargetName = isClient 
        ? (user?.socialAccounts && user.socialAccounts.length > 0 ? `@${user.socialAccounts[0].username}` : user?.name) 
        : 'Viraliza.ai';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1">
                     <div className="bg-secondary p-6 rounded-lg">
                        <div className="flex justify-between items-center">
                            <h3 className="text-xl font-bold flex items-center gap-2"><RocketIcon className="w-6 h-6 text-accent"/> Status do Piloto</h3>
                            <label className="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" checked={isActive} onChange={(e) => canActivate && setIsActive(e.target.checked)} className="sr-only peer" disabled={!canActivate} />
                                <div className={`w-11 h-6 bg-primary peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all ${canActivate ? 'peer-checked:bg-accent' : 'bg-gray-700'}`}></div>
                            </label>
                        </div>
                        <p className={`mt-2 font-semibold text-lg ${isActive ? 'text-green-400' : 'text-red-400'}`}>{isActive ? 'Ativo e Promovendo' : 'Inativo'}</p>
                        <p className="text-sm text-gray-dark mt-4">Quando ativo, a IA executa autonomamente ações de marketing para aumentar a visibilidade e atrair novos clientes. Novas ações são geradas a cada 10 segundos.</p>
                        {!canActivate && (
                ...
                <div className="lg:col-span-2">
                    <div className="bg-secondary p-6 rounded-lg">
                        ...
                        <div ref={logsContainerRef} className="bg-primary p-4 rounded-lg h-[60vh] max-h-[500px] overflow-y-auto flex flex-col-reverse">
                            <div className="space-y-4">
                                {!isActive && actionLogs.length === 0 && (
                                    <p className="text-center text-gray-dark p-8">
                                        {canActivate ? 'Ative o piloto para iniciar o marketing autônomo.' : 'Preencha os dados do seu negócio no Dashboard.'}
                                    </p>
                                )}
                                {actionLogs.map(log => (
