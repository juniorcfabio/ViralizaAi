...
const CopyIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>;
...
const AffiliatePage: React.FC = () => {
        ...
        return (
            <div className="relative h-full">
                <header className="mb-8">
                    ...
                    <p className="text-gray-dark">Ganhe comissões indicando novos clientes para a Viraliza.ai.</p>
                </header>
                <FeatureLockedOverlay featureName="Programa de Afiliados" requiredPlan="Plano Semestral" />
            </div>
        );
    }
    ...
    useEffect(() => {
        if (!isAffiliate || referredUsers.length === 0) return;

        const planPrices: { [key: string]: number } = {
            'Anual': 399.90, 'Semestral': 259.90, 'Trimestral': 159.90, 'Mensal': 59.90,
        };
        const commissionRate = parseFloat(localStorage.getItem('viraliza_affiliate_commission_rate') || '20') / 100;

        const dataByMonth: { [key: string]: { totalReferrals: number, convertedReferrals: number, monthlyEarnings: number } } = {};
        ...
        referredUsers.forEach(refUser => {
            ...
            if (!dataByMonth[monthKey]) {
                dataByMonth[monthKey] = { totalReferrals: 0, convertedReferrals: 0, monthlyEarnings: 0 };
            }
        ...
        <>
            ...
            {!isAffiliate ? (
                <div className="bg-secondary p-8 rounded-lg text-center max-w-2xl mx-auto">
                    ...
                    <p className="text-gray-dark mb-6">Ative sua conta de afiliado para receber um link exclusivo e comece a ganhar comissões por cada novo cliente que você indicar.</p>
                    <button onClick={handleActivate} className="bg-accent text-light font-semibold py-3 px-8 rounded-full hover:bg-blue-500 transition-colors">
                ...
                <div className="space-y-8">
                    <div className="bg-secondary p-6 rounded-lg max-w-sm ml-auto">
                        ...
                        <div className="relative">
                            ...
                            />
                            <button onClick={handleCopyLink} className="absolute inset-y-0 right-0 flex items-center px-3 text-gray-dark hover:text-light">
                                <CopyIcon className="w-4 h-4"/>
                            </button>
                        </div>
                        {linkCopied && <p className="text-green-400 text-xs mt-1 text-center">Link copiado!</p>}
                    </div>
                        ...
                        <div className="lg:col-span-2 bg-secondary p-6 rounded-lg">
                            ...
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                    ...
                                    <Legend />
                                    <Line type="monotone" dataKey="Ganhos Acumulados" stroke="#4F46E5" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }}/>
                                    <Line type="monotone" dataKey="Ganhos Mensais" stroke="#818CF8" strokeWidth={2} strokeDasharray="3 3" />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                            ...
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody>
                                    {referredUsers.length === 0 && (
                                        <tr>
                                            <td colSpan={4} className="text-center p-6 text-gray-dark">Nenhum usuário indicado ainda. Compartilhe seu link!</td>
                                        </tr>
                                    )}
                                    {referredUsers.map(refUser => (
                                        <tr key={refUser.id} className="border-t border-primary">
                                            <td className="p-3 font-medium">{refUser.name}</td>
                                            <td className="p-3">{refUser.plan || 'Aguardando Assinatura'}</td>
                                            <td className="p-3">{new Date(refUser.joinedDate).toLocaleDateString('pt-BR')}</td>
                                            <td className="p-3">
                                                 <span className={`px-2 py-1 text-xs rounded-full ${refUser.plan ? 'bg-green-500/20 text-green-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
