...
const AddProfileModal: React.FC<{ onClose: () => void; onAdd: (platform: string, username: string) => void }> = ({ onClose, onAdd }) => {
            ...
            <div className="bg-secondary p-8 rounded-lg shadow-xl w-full max-w-sm relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <select value={platform} onChange={e => setPlatform(e.target.value)} className="w-full bg-primary p-3 rounded border border-gray-600 focus:outline-none focus:ring-2 focus:ring-accent">
        ...
        <>
            ...
            <header className="flex justify-between items-center mb-8 flex-wrap">
                <div>
                    ...
                    <p className="text-gray-dark">Informe à IA quais são seus perfis para gerar estratégias personalizadas.</p>
                </div>
                <button 
                    onClick={() => checkAccess() && setIsModalOpen(true)}
                    className={`bg-accent text-light font-semibold py-2 px-6 rounded-full hover:bg-blue-500 transition-colors ${!isActive ? 'opacity-70' : ''}`}
                >
                    Adicionar Perfil
                </button>
            </header>
            ...
            <div className="bg-primary/50 border border-accent/30 p-4 rounded-lg mb-8">
                ...
                <p className="text-sm text-gray-dark mt-1">
                    Ao informar seus perfis, você fornece à nossa IA o contexto necessário para analisar seu nicho e criar campanhas de crescimento e funis de venda muito mais eficazes e alinhados com sua marca. Não armazenamos senhas nem postamos em seu nome.
                </p>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody>
                            {accounts.length === 0 && (
                                <tr>
                                    <td colSpan={3} className="text-center p-8 text-gray-dark">Nenhum perfil de marca adicionado.</td>
                                </tr>
                            )}
                            {accounts.map(acc => (
                                <tr key={acc.id} className="border-b border-primary hover:bg-primary">
                                    ...
                                    <td className="p-4">
                                        <button onClick={() => handleRemove(acc.id)} className="text-red-500 hover:text-red-400">Remover</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {isModalOpen && <AddProfileModal onClose={() => setIsModalOpen(false)} onAdd={handleAddAccount} />}
        </>
    );
};
