...
const ExpandIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/></svg>
);
...
const AnalyticsPage: React.FC = () => {
    ...
    const [campaignPerformanceData, setCampaignPerformanceData] = useState<{ name: string, engagement: number }[]>([]);
    ...
    useEffect(() => {
        ...
        if (storedData) {
            ...
            setReachData(data.reachData);
            setSalesFunnelData(data.salesFunnelData);
            setPostReachData(data.postReachData || []);
            setCampaignPerformanceData(data.campaignPerformanceData || []);
        } else {
            setEngagementData([{ name: 'Mês 1', rate: 0.5 }]);
            setReachData([
                { name: 'TikTok', reach: 1250 }, { name: 'Facebook', reach: 800 },
                { name: 'Instagram', reach: 2300 }, { name: 'X', reach: 450 },
                { name: 'Pinterest', reach: 600 }, { name: 'YouTube', reach: 900 }, { name: 'Telegram', reach: 200 }
            ]);
            setSalesFunnelData([
        ...
        const interval = setInterval(() => {
            setEngagementData(prev => {
                ...
                const newPoint = { name: `Mês ${prev.length + 1}`, rate: parseFloat(newRate.toFixed(2)) };
                return [...prev, newPoint].slice(-6);
            });

            setReachData(prev => prev.map(item => ({
                ...item,
            ...
            setSalesFunnelData(prev => {
                 if (prev.length === 0) return prev;
                const views = prev[0].value + Math.floor(Math.random() * 2000);
                const clicks = prev[1].value + Math.floor(Math.random() * 60);
                const sales = prev[2].value + (Math.random() > 0.8 ? Math.floor(Math.random() * 5) : 0);
                return [
            ...
            setPostReachData(prev => prev.map(post => ({
                ...post,
                reach: post.reach + Math.floor(Math.random() * 25)
            })).sort((a,b) => b.reach - a.reach));

            setCampaignPerformanceData(prev => prev.map(campaign => ({
                ...campaign,
    ...
    useEffect(() => {
        if(user && hasAccess('analytics')) {
             const dataKey = `viraliza_analytics_data_${user.id}`;
             const dataToStore = { engagementData, reachData, salesFunnelData, postReachData, campaignPerformanceData };
             localStorage.setItem(dataKey, JSON.stringify(dataToStore));
        }
    }, [engagementData, reachData, salesFunnelData, postReachData, campaignPerformanceData, user, hasAccess]);
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-secondary p-6 rounded-lg">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={engagementData}>
                            ...
                            <Legend />
                            <Line type="monotone" dataKey="rate" name="Engajamento" stroke="#4F46E5" strokeWidth={2} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                ...
                <div className="bg-secondary p-6 rounded-lg">
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={reachData} layout="vertical">
                            ...
                            <Tooltip contentStyle={{ backgroundColor: '#02042B', border: '1px solid #0B1747' }} formatter={(value) => new Intl.NumberFormat('pt-BR').format(value as number)} />
                            <Legend />
             ...
             {/* Full-screen modal for Reach Chart */}
                    ...
                    <div className="bg-secondary rounded-lg shadow-xl w-full max-w-4xl h-auto max-h-[90vh] flex flex-col">
                        <div className="flex justify-between items-center p-4 border-b border-primary">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => setIsReachChartModalOpen(false)} className="text-2xl text-gray-dark hover:text-light">&times;</button>
                            </div>
                        </div>

                        <div className="flex-1 p-6" style={{ height: '70vh' }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={reachData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" stroke="#0B1747" />
                                    <XAxis type="number" stroke="#94A3B8" />
                                    <YAxis type="category" dataKey="name" stroke="#94A3B8" width={80} />
                                    <Tooltip contentStyle={{ backgroundColor: '#02042B', border: '1px solid #0B1747' }} />
