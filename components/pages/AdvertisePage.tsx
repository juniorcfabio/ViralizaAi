...
const AdvertisePage: React.FC = () => {
    ...
    const [pricing] = useState<AdPricingConfig>(getAdPricingConfig());
        ...
        <div className="max-w-4xl mx-auto">
            <header className="mb-8 text-center">
                ...
                <p className="text-gray-dark mt-2">
                    Destaque sua empresa na nossa página inicial e alcance milhares de visitantes
                    diários.
                </p>
            </header>
                ...
                <div className="bg-secondary p-8 rounded-lg border border-gray-700 animate-fade-in-up">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm text-gray-dark mb-1">
                                    Cargo / Slogan (Aparece abaixo do nome)
                                </label>
                                <input
                                    type="text"
                                    value={formData.role || ''}
                    ...
                    <div className="mt-8 text-right">
                        <button
                            disabled={
                                !formData.companyName || !formData.logo || !formData.websiteUrl
                            }
                            onClick={() => setStep(2)}
            ...
            {step === 2 && (
                <div className="animate-fade-in-up">
                    <h3 className="text-xl font-bold mb-6 text-center">
                        2. Escolha o Período de Exposição
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div
                            onClick={() => setSelectedPlan('1_week')}
                            className={`bg-secondary p-6 rounded-lg border-2 cursor-pointer transition-all transform hover:-translate-y-1 ${
                                selectedPlan === '1_week'
                        ...
                        >
                            ...
                            <p className="text-3xl font-bold text-light mt-2">
                                R$ {pricing.oneWeek.toFixed(2)}
                            </p>
                            <p className="text-sm text-gray-500 mt-2">
                                Visibilidade rápida para promoções.
                            </p>
                        </div>
                        <div
                        ...
                        >
                            <div className="bg-accent text-white text-xs px-2 py-1 rounded-full inline-block mb-2">
                                Popular
                            </div>
                            <h4 className="text-lg font-bold text-gray-300">15 Dias</h4>
                            <p className="text-3xl font-bold text-light mt-2">
                                R$ {pricing.fifteenDays.toFixed(2)}
                            </p>
                            <p className="text-sm text-gray-500 mt-2">
                                Ideal para campanhas de médio prazo.
                            </p>
                        </div>
                        <div
                            onClick={() => setSelectedPlan('30_days')}
                            className={`bg-secondary p-6 rounded-lg border-2 cursor-pointer transition-all transform hover:-translate-y-1 ${
                                selectedPlan === '30_days'
                                    ? 'border-accent shadow-lg shadow-accent/20'
                                    : 'border-gray-700 hover:border-gray-500'
                            }`}
                        >
                            ...
                            <p className="text-3xl font-bold text-light mt-2">
                                R$ {pricing.thirtyDays.toFixed(2)}
                            </p>
                            <p className="text-sm text-gray-500 mt-2">
                                Máxima exposição e autoridade.
                            </p>
                        </div>
                    </div>
                    ...
                    {selectedPlan && (
                        <div className="bg-primary p-6 rounded-lg border border-gray-700 text-center">
                            <p className="mb-4 text-gray-dark">
                                Resumo: Anúncio de <strong>{formData.companyName}</strong> por{' '}
                                <strong>
                                    {selectedPlan === '1_week'
                                        ? '7 dias'
                                        : selectedPlan === '15_days'
                            ...
                            <button
                                onClick={() => setStep(1)}
                                className="text-sm text-gray-400 mt-4 hover:text-white"
                            >
                                ← Voltar
                            </button>
                        </div>
